...
public class ResponseImportSelector implements ImportSelector {
    ...
    static {
        ...
        ALL_CONFIGURATION.put("useResponse", ResponseConfiguration.class.getName());
    }

    @Override
    public String[] selectImports(AnnotationMetadata importingClassMetadata) {
        // 当存在注解 EnableResponse 时   取其useXxx 布尔值  true  导入 XxxConfiguration
        // 其它任何情况将不导入
        if (importingClassMetadata.hasAnnotation(EnableResponse.class.getName())) {
            List<String> importClassNames = new ArrayList<>();
            MultiValueMap<String, Object> attributes = importingClassMetadata.getAllAnnotationAttributes(EnableResponse.class.getName());
